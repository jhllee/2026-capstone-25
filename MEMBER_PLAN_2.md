...
| 하위 단계로 쪼개기 프론트 UI (상세 화면 통합 + 하위 단계 박스 누적) | 완전 | 재은 |
...
### R7. 하위 단계로 쪼개기 프론트 통합 — **재은**

| 항목 | 내용 |
|---|---|
| **작업일** | _일 |
| **선행 조건** | 백엔드 `POST /api/decompose/sub`는 v1에서 완료. 상세 화면(J4)도 완료 |
| **검증** | 상세 화면에서 단계의 "하위 단계로 쪼개기" 클릭 → 3~5개 하위 단계가 박스로 누적 → 새로고침 후 유지. 하위 모두 완료 시 본 단계 자동 완료 |
| **PR 브랜치** | `feat/sub-decompose-ui` |

**산출물**:
- [x] `components/detail/StepRow.tsx` 확장 — 하위 없는 단계에 "하위 단계로 쪼개기" 버튼 노출
...
10. 다른 단계의 "하위 단계로 쪼개기" → 하위 3~5개 박스 표시 → 하위 하나 체크
